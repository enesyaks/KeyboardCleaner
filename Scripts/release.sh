...
# Updates Casks/kbcler.rb with version + sha256.
#
# Prerequisites:
...
CASK_PATH="$ROOT/Casks/kbcler.rb"
TAG="v${VERSION}"
SKIP_UPLOAD=0
...
if ! command -v gh >/dev/null 2>&1; then
  ...
  echo "      brew install --cask kbcler"
  exit 0
fi

cd "$ROOT"
if ! git diff --quiet -- "$CASK_PATH" 2>/dev/null || ! git diff --cached --quiet -- "$CASK_PATH" 2>/dev/null; then
  echo "==> Tip: commit the updated Casks/kbcler.rb before or after the release"
fi
